...
Uses Jetpack Navigation Compose (`NavHost` + `NavController`) with 4 bottom tabs (计划/记账/资产/我的). The `AppNavigation.kt` composable orchestrates all ViewModels, the QuickAdd drawer, the numeric keypad overlay, and voice input. `navigation-compose` is actively used.
