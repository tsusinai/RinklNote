 ...
 *
 * 2026-09-14 按使用逻辑重排分组标题（卡片在页面的先后顺序由 ProfileScreen 的 LazyColumn 决定）：
 * 数据与同步 → 通知 → 账户与安全 → 通用；卡内行序同样按使用频率从高到低排列。
 */
...
/**
 * «账户与安全» 组：按使用频率排列行序——高频的 AI 推送开关、主题与背景在前，
 * 低频的接口配置、密码与 QQ 绑定类垫后。
 *
